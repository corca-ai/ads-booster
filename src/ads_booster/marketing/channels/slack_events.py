...
다시 시작 — 닫힌 대화 재개
첨부파일은 이 대화의 접근 범위에 맞춰 지식으로 저장합니다."""
...
class SlackEvents:
    ...
    def _message(  # noqa: C901,PLR0911 - explicit channel and DM admission boundaries.
        ...
        action = (
            "input" if run is not None and run.state is AgentRunState.AWAITING_INPUT else "create"
        )
        run_id = (
            run.run_id
    ...
    def _receive_mutation(self, event: JsonObject, *, now: datetime) -> None:
        ...
        knowledge = build_slack_ingress(
            SlackIngressRequest(
                ...
                run_id=previous_binding.run_id,
                action="input",
                text=text,
    ...
    def work_once(self, *, now: datetime) -> bool:
        ...
        try:
            ...
            with service.execution_lock:
                ...
                if plan.action in {"create", "input", "resume"} and self.store.claim_ack(message):
                    self.store.sent(
                        message,
                        self._send(
