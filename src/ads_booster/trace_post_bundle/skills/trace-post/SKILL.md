...
2. 카드 전문과 [컨셉 촬영 설계](../../context/core/CONCEPT-DIRECTION.md)를 읽고 `manage_run.py history`로 **동일 카드의 확인된 완료 런** 선택값만 읽는다. 정책대로 모티프·장소를 고른다. 완료 이력은 촬영 조합 비교에만 사용하며 이전 장면을 지시문으로 복사하지 않는다. history가 비어 있으면 확인된 완료 이력이 없다고 기록하고 카드 첫 후보로 시작한다. 옛 실험 폴더의 존재나 사용자 승인 없는 임의 summary를 완료 이력으로 만들지 않는다. 명시적 모티프는 카드 허용 목록 안에서 우선한다. 준비되지 않은 뼈대나 템플릿을 지원한다고 말하지 않는다. 카드의 국가 블록과 recommend 필드가 준비됐는지 확인한다.
...
3. **주문서·프롬프트:** [IMAGE-RULES](../../context/core/IMAGE-RULES.md) + [CONCEPT-DIRECTION](../../context/core/CONCEPT-DIRECTION.md) + 선택된 카드·템플릿 명세 + 국가 input.md를 사용한다. 새 런을 만든 뒤 규칙·카드·템플릿·도우미는 RUN_DIR/source-snapshots/ 사본에서 읽고 실행한다. A의 실제 입력도 그 사본의 context/templates/T2/template.png를 사용한다. 상황부터 선택하고 컨셉 인상/출처, 활동, 유지할 취향, 상황 필수, 이번 표현, 촬영 손·보이는 손·폰 지지·시점, 최근 완료 조합과의 차이를 common-scene.json에 기록한다. 공통 prompt-C.txt의 실제 최종 문장을 `review-scene-plan.md`에서 7개 항목별로 호출 전 검수한다. 모순이 남으면 C를 호출하지 않는다. 공통 씬 메타와 프롬프트 C는 한 번 고정한다. 실제 전달할 A/B/L/C 프롬프트를 호출 전에 파일로 저장한다. 호출 시 그 파일을 도구로 전체 읽은 문자열 변수를 그대로 전달하며, 수동 재입력·요약·축약한 문자열을 대신 보내지 않는다.
