...
6. {distinct}
7. 배경화면이 드러내는 것은 **그 사람이 무엇을 좋아하는가**입니다. 무슨 일이 벌어지는
   장면이 아닙니다. 직업과 하루는 trace_items와 캡션이 이미 드러냅니다.
   - background_subject: 기계가 읽는 토큰입니다. 그 사람이 실제로 잠금화면에 설정해뒀을
     법한 배경을 아래 목록에서만 고르세요. 토큰 외의 값이나 새 단어를 만들지 마세요.
     {subjects}
   - background_search_query: 이미지 검색에 그대로 들어갈 **대상의 이름**입니다.
   - background_mood: 그 배경의 색·톤 한마디입니다. 장면이 아닙니다.
   query 를 먼저 정하고 mood 를 그다음에 쓰세요.
8. image_inputs.background_search_query 는 **"이름 + 배경화면"** 입니다. 30자 안에서
   쓰세요.
   - **이름은 계정 블록의 관심사에서 고르세요.** 그 사람이 좋아하는 것 하나를 대는
     자리입니다. 관심사가 없으면 그 페르소나가 좋아할 법한 것을 정하세요.
     기아 팬 → "김도영 배경화면", 미대생 → "클림트 키스 배경화면",
     산리오 좋아하는 사람 → "쿠로미 배경화면", 등산 다니는 사람 → "북한산 배경화면".
     위 예시는 형식 참고용입니다. 예시의 인물·작품·캐릭터를 그대로 쓰지 말고 새로 정하세요.
   - 이 필드에 한해 실존 인물명·캐릭터명·팀명·작품명을 그대로 써도 됩니다. 고유명사를
     쓰는 자리는 여기 하나뿐입니다. 고유명사가 곧 그 사람의 개성이고, 이미지 색인이
     실제로 사진에 붙여둔 단어라서 검색도 가장 잘 됩니다.
   - **장면을 쓰지 마세요.** 서술어가 들어가면 그것은 이름이 아니라 문장입니다. 이미지
     검색은 문장을 이해하지 못하고, 워터마크 박힌 클립아트와 스톡 모델 사진을 돌려줍니다.
     나쁜 예: "해 질 무렵 그네 옆을 뛰어가는 남매 배경화면"
     좋은 예: "곰돌이 푸 배경화면"
   - **반드시 "배경화면"으로 끝납니다.** 이 단어가 없으면 같은 말이 웹 전체를 검색합니다.
     "김도영"은 가로로 길고 작은 뉴스 사진을 돌려주고 해상도 관문에서 한 장도 남지
     않습니다. "배경화면"이 붙어야 검색이 폰 배경화면 영역으로 들어가고, 거기 있는 사진은
     이미 세로입니다.
   - "배경화면"을 포함해 2~4단어입니다. 즉 이름은 1~3단어입니다. 단어가 늘어날수록
     결과가 뉴스 기사 사진 쪽으로 밀립니다.
     나쁜 예: "KIA 타이거즈 야간 경기장 외야 관중석 배경화면"
     좋은 예: "KIA 타이거즈 배경화면"
   - background_subject와 정합해야 합니다. scenery면 풍경, pet이면 반려동물,
     character_*면 그 캐릭터, sports_team이면 그 팀이나 선수입니다.
   - 직업 소품·작업 공간을 쓰지 마세요. 아무도 자기 일터를 찍은 정물 사진을 배경화면으로
     깔지 않습니다. 간호사라고 병원 사진, 카페 사장이라고 카페 사진을 깔지 않습니다.
     나쁜 예: "병원 사물함 간호사 명찰 볼펜 사진"
   - "감성 배경", "예쁜 사진" 같은 범용 검색어도 금지입니다. 아무 이름도 대지 않은 것과
     같습니다. "고화질", "HD" 같은 해상도 단어도 붙이지 마세요. 그 단어는 데스크톱
     월페이퍼 수집 사이트로 검색을 밀어서 1920x1080 가로 사진만 돌아옵니다.
9. image_inputs.background_mood 는 그 배경의 **색과 톤 한마디**입니다. 12자 안에서
   쓰세요. 예: "따뜻한 파스텔", "짙은 남색", "차분한 밤".
   장면을 쓰는 자리가 아닙니다. 사람이 읽는 설명이고 검색에는 쓰이지 않습니다.
...
3. 배경 검색어의 이름은 위 관심사 또는 배경 취향에서 고르세요. 그 사람이 실제로
   좋아하는 것 하나를 대는 자리입니다.
...
def build_instruction(  # noqa: PLR0913 - each argument is one independent prompt section.
    ...
    *,
    ...
    subjects = ", ".join(subject.value for subject in OFFERED_BACKGROUND_SUBJECTS)
    one = count == 1
