...
8. image_inputs.background_search_query 는 "그 사람의 직업이나 상황을 묘사하는 사진"이
   ...
   - **검색어는 반드시 "배경화면"으로 끝납니다.** 이 단어가 없으면 같은 말이 웹 전체를
     검색합니다. "김도영"은 가로로 길고 작은 뉴스 사진을 돌려주고 해상도 관문에서 한 장도
     남지 않습니다. "배경화면"이 붙어야 검색이 폰 배경화면 영역으로 들어가고, 거기 있는
     사진은 이미 세로입니다.
   - 그러므로 앞부분은 **"배경화면"을 붙여도 말이 되는 이름**이어야 합니다. 이름 또는
     명사구로 쓰세요. 서술어나 문장이 앞에 오면 붙였을 때 말이 안 됩니다.
     나쁜 예: "해 질 무렵 그네 옆을 뛰어가는 남매 배경화면"
     좋은 예: "노을 놀이터 배경화면"
   - 검색어는 "배경화면"을 포함해 2~4단어입니다. 즉 앞에 오는 이름은 1~3단어입니다.
     단어가 늘어날수록 결과가 뉴스 기사 사진 쪽으로 밀립니다.
