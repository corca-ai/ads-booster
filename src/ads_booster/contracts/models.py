...
from ads_booster.contracts.limits import MAX_TRACE_ITEMS, TRACE_ITEM_COUNT
...
COMPONENT_LAYOUT_MISMATCH: Final = "component_layout_mismatch"
COMPONENT_LAYOUT_MISMATCH_MESSAGE: Final = "component count must match the declared Trace layout"
...
class TraceComponentLayout(StrEnum):
    ...
    def component_count(self) -> int:
        match self:
            case TraceComponentLayout.ONE_BY_ONE:
                return 1
            case TraceComponentLayout.TWO_BY_ONE:
                return 2
            case TraceComponentLayout.TWO_TOP_ONE_BOTTOM:
                return 3
            case TraceComponentLayout.TWO_BY_TWO:
                return 4
            case unreachable:
                assert_never(unreachable)


class TraceComponent(ContractModel):
    title: TraceItem
    items: Annotated[tuple[TraceItem, ...], Field(min_length=1, max_length=4)]


class TraceComponentRow(ContractModel):
    layout: TraceComponentLayout
    components: Annotated[tuple[TraceComponent, ...], Field(min_length=1, max_length=4)]
    ...
    def require_layout_component_count(self) -> Self:
        if len(self.components) != self.layout.component_count:
            raise PydanticCustomError(
                ...
                COMPONENT_LAYOUT_MISMATCH_MESSAGE,
            )
        return self


class TraceData(ContractModel):
    rows: Annotated[tuple[TraceComponentRow, ...], Field(min_length=1, max_length=4)]

    @property
    def items(self) -> tuple[str, ...]:
        return tuple(
            item for row in self.rows for component in row.components for item in component.items
        )

    @model_validator(mode="after")
    def require_one_to_eight_items(self) -> Self:
        item_count = sum(len(component.items) for row in self.rows for component in row.components)
        if not 1 <= item_count <= MAX_TRACE_ITEMS:
            raise PydanticCustomError(
                TRACE_ITEM_COUNT,
                "Trace component rows must contain between one and eight items",
            )
        return self
...
class CaptureScene(ContractModel):
    scene_id: Identifier
    locale: Locale
    capture_target: Literal["trace_components", "trace_wallpaper"]
...
class CaptureResult(ContractModel):
    ...
    def from_captures(
        ...
        if completed_count == len(captures):
            return cls(job_id=job_id, status=JobStatus.COMPLETED, captures=captures, errors=errors)
        if completed_count == 0:
            return cls(job_id=job_id, status=JobStatus.FAILED, captures=captures, errors=errors)
        return cls(job_id=job_id, status=JobStatus.PARTIAL, captures=captures, errors=errors)
