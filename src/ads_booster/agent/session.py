...
from ads_booster.agent.memory import MemoryStore, MemoryStoreError, NullMemoryStore
...
class AgentSession:
    ...
    def ask(
        ...
        prompt: str,
        *,
        on_tool: Callable[[ToolExecutionEvent], None] | None = None,
    ) -> str:
        self.history.append({"role": "user", "content": prompt})
        overflow_retried = False
        while True:
            decision = self.context_runtime.prepare([*self.context_prefix, *self.history])
            ...
            try:
                ...
                decision = self.context_runtime.force_compact(
                    [*self.context_prefix, *self.history],
            ...
            if turn.calls:
                self._append_calls(turn, on_tool)
                continue
    ...
    def _append_calls(
        self,
        turn: ModelTurn,
        on_tool: Callable[[ToolExecutionEvent], None] | None,
    ) -> None:
        if turn.text:
            self.history.append({"role": "assistant", "content": turn.text})
        for call in turn.calls:
            ...
            if result.ok:
                output = result.model_output_json() if result.model_output else result.output
            else:
                output = f"[{result.error_code}] {result.output}"
            self.history.append(
                {
