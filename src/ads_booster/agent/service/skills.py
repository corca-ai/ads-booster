...
_MARKETING_SKILLS = (
    ...
    MarketingSkill(
        ...
        required_capabilities=("creative.trace_post",),
        success_criteria=(
            "새 일정·국가별 캡션과 KR·JP·TW 잠금화면 및 장면 6개의 검증된 자산이 있다.",
            "검증된 이미지와 캡션을 요청한 대화에 전달한다. 사람의 피드백은 선택이다.",
        ),
        procedure=(
            "1. 귀여움 카드의 T2 게시물 요청에 사용한다. creative.trace_post의 현재 입력 스키마와 "
            "사용자가 지정한 날짜·모티프·환경 앵커를 확인한다. 세부 촬영 상황은 카드의 합의된 "
            "취향과 완료 이력의 반복 비교를 바탕으로 먼저 자동 설계한다. 지원하지 않는 카드나 "
            "템플릿은 지원한다고 추정하지 않는다.\n"
            "2. creative.trace_post를 통해 설치 패키지에 포함된 규칙과 템플릿으로 실행한다. "
            "사용자의 제작 요청으로 바로 실행하며 추가 승인이나 검토를 요구하지 않는다. "
            "날짜·모티프·장소를 맡기면 지원 범위에서 선택한다. 도구가 없으면 알려주고 "
            "일반 이미지 생성으로 같은 파이프라인을 실행했다고 말하지 않는다.\n"
            "3. 실행은 새 일정·캡션·현지화를 검수하고 KR A→B, 동일한 KR 완성 배경에서 "
            "JP/TW L, 국가별 공통 프롬프트 C 순서로 생성한다. C는 활동 중 순간, 셀프 촬영 가능한 "
            "손·폰 배치, 컨셉 취향과 게시물별 표현을 분리해 호출 전에 검수한다. 화면의 일정·캡션은 "
            "현지화하며 손·손톱·장면의 픽셀 동일성을 요구하지 않는다. 실제 지침은 실행별 동결 "
            "사본을 따른다.\n"
            "4. 비동기 결과를 기다리고 검증된 자산과 검수·오류 기록을 확인한다. "
            "생성 결과가 불명하면 재생성을 요청하지 않고 기존 작업의 확인 상태를 안내한다. "
            "최종 이미지 6개가 검증되기 전에 완성했다고 말하지 않는다.\n"
            "5. 결과의 캡션과 국가별 이미지 6개를 요청한 Slack 대화에 첨부해 "
            "바로 보고 다운로드할 수 있게 전달한다. 사람의 피드백은 선택이며 "
            "검토·승인·상태 명령이나 운영자 문의를 필수 단계로 안내하지 않는다. "
            "배경 픽셀 완전 동일성이나 실제 앱 화면임을 검증 없이 주장하지 않는다. "
            "요청한 대화에 결과를 반환하는 것과 별도 채널 전달·Threads 게시는 구분한다."
        ),
    ),
    MarketingSkill(
    ...
    MarketingSkill(
        ...
        required_capabilities=(),
        success_criteria=(
            "근거에 맞는 추천과 목표·측정 가능성·실행 순서·검수 조건을 갖춘 계획이 있다.",
        ),
        procedure=(
            ...
            "캠페인 계획이면 주지표의 정의·분모·측정 출처·관찰 기간·판단 시점을 정한다. "
            "기준 성과나 목표값이 없으면 미정 또는 제안값으로 표시하며 업계 수치를 만들지 않는다. "
            "목표 행동을 귀속·추적할 수 없으면 대리 지표의 학습과 사업 성과 검증을 구분한다.\n"
            "5. 제공된 예산·제작 및 검수 시간을 지키는 최소 실행 순서를 작성한다. "
            "각 작업의 자산·담당·기한·선행 조건을 연결하고 미정 담당자를 임의 배정하지 않는다. "
            "측정 준비·제품 주장 검수·필요 승인을 실행보다 먼저 배치한다. "
            "배포 일정과 관찰 완료 시점은 구분하고 다음 판단을 바꿀 조건을 적는다. "
            "간단한 문구 요청이면 계획표 없이 바로 문구를 작성한다. "
            "정식 제작안 저장이 필요할 때만 "
    ...
    MarketingSkill(
        ...
        version="1",
        purpose="주간·월간·캠페인 성과 보고서를 지표 정의·비교 한계·우선 조치와 함께 작성한다.",
        required_capabilities=(),
        success_criteria=(
            "재계산 가능한 지표와 누락·비교 한계, 목표에 맞는 다음 조치가 있는 보고서가 있다.",
        ),
        procedure=(
            "1. 보고 대상·사업 목표·기간·시간대·계정·국가·채널·데이터 출처를 확인한다. "
            "이미 제공된 정보를 다시 묻지 않는다. 연결된 도구의 실제 범위만 조회하며 "
            "사람이 보고한 집계를 직접 수집한 플랫폼 근거로 바꾸지 않는다.\n"
            "2. 계산 전에 지표 정의·분자·분모·고유 인원과 반복 이벤트·중복·누락·통화·"
            "귀속 방식·관찰 완료 여부를 확인한다. 중복 제거 기준이 없으면 임의로 합치지 않는다. "
            "누락된 비용·매출·이전 기간·목표는 미상이며 0이 아니다.\n"
            "3. 성숙한 동일 코호트의 중첩 고유 인원은 사용 가능한 marketing.analyze로 "
            "계산한다. 노출·반복 클릭을 고유 인원 퍼널에 넣지 않는다. "
            "합산 전환율은 정의가 같고 서로 겹치지 않는 집계의 분자 합/분모 합으로 계산하며 "
            "행별 비율의 단순 평균을 쓰지 않는다. 식과 입력을 보여 재계산 가능하게 한다. "
            "0 분모는 미정의다. 비율 차이는 퍼센트포인트, 상대 변화는 기준값 대비 %로 "
            "구분하며 이전 값이 0이거나 없으면 상대 증감률을 만들지 않는다.\n"
            "4. 핵심 결론 → 목표 관련 지표 → 관찰과 설명 가설 → 다음 우선 조치로 보고한다. "
            "표에는 필요한 현재값·비교값·목표·판단만 넣는다. 비교 기간이나 목표가 없으면 "
            "증감·목표 달성 상태를 꾸미지 않는다. 비용·매출·귀속 근거 없이 ROI를 만들거나 "
            "조회수 상승을 매출 성과로 바꾸지 않는다. 집단 구성·기간·배포량의 차이를 "
            "대안 설명으로 검토하고 상관을 원인이나 증액 효과로 확정하지 않는다.\n"
            "5. 영향·근거 확실성·작업량을 보고 가장 유용한 다음 조치를 우선한다. "
            "측정 공백이 판단을 막으면 확인할 데이터와 담당 미정 여부·판단 시점을 적고 "
            "가능한 부분 보고는 완성한다. 보고서 작성은 게시·예산 집행·공용 기억 저장 "
            "승인이 아니다. 실행 완료나 검증된 학습은 실제 receipt와 관찰 근거로만 말한다."
        ),
    ),
    MarketingSkill(
        ...
        procedure=(
            ...
            "독자가 해결하려는 질문에 실제로 도움이 되는 구체적인 내용을 담는다. "
            "검색용 글도 키워드 반복·분량 채우기·타사 요약 재작성만으로 끝내지 않는다. "
            "제목과 본문이 약속하는 내용을 맞추고 직접 써 본 경험·후기·순위·전문성을 "
            "꾸미지 않는다. 제공된 근거와 우리 제품의 적용 범위로 가치를 더한다.\n"
