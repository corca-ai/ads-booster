...
ACCEPT_REVIEW: Final = _sql(
    "UPDATE automation_queue SET state = 'accepted', revision = revision + 1, updated_at = ?",
    "WHERE queue_id = ? AND workspace_id = ? AND state = 'review' AND revision = ?",
)
REQUEUE_REJECTED_REVIEW: Final = _sql(
    "UPDATE automation_queue SET state = 'submitted', attempts = 0, worker_id = NULL,",
    "lease_until = NULL, run_id = NULL, run_idempotency_key = NULL, artifact_path = NULL,",
    "artifact_sha256 = NULL, failure_code = NULL, revision = revision + 1, updated_at = ?",
    "WHERE queue_id = ? AND workspace_id = ? AND state = 'review' AND revision = ?",
)
