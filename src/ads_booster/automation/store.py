...
from ads_booster.automation.models import (
    QueueCompletion,
    QueueId,
    QueueRecord,
    QueueState,
...
from ads_booster.automation.queries import (
    ACCEPT_REVIEW,
    CLAIM,
    EXHAUST_CLAIM,
    EXPIRE_RUNNING,
...
from ads_booster.automation.rows import parse_row, queue_record
...
class AutomationQueue:
    ...
    def claim_due(
        ...
        with self._database.connect(write=True) as connection:
            ...
            if candidate is None:
                return None
            if len(candidate) != 1 or not isinstance(candidate[0], str):
                msg = "automation queue candidate row is corrupt"
                raise RuntimeError(msg)
            queue_id = candidate[0]
    ...
    def finish(
        ...
        *,
        ...
        if completion.state not in (QueueState.REVIEW, QueueState.FAILED):
            raise InvalidQueueCompletionError(completion.state)
        with self._database.connect(write=True) as connection:
            result = connection.execute(
                FINISH,
                (
    ...
    def review(
        ...
        *,
        ...
        statement = ACCEPT_REVIEW if accepted else REQUEUE_REJECTED_REVIEW
        with self._database.connect(write=True) as connection:
            result = connection.execute(
                statement,
                (now.timestamp(), queue_id, workspace_id, expected_revision),
            )
            if result.rowcount != 1:
                raise QueueRevisionError(queue_id, expected_revision)
