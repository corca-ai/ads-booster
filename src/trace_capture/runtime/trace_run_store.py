...
from trace_capture.runtime.trace_run_artifacts import prepare_artifact, request_digest
from trace_capture.runtime.trace_run_lock import JOURNAL_FILENAME, RunLock, StoreLock
from trace_capture.runtime.trace_run_replay import (
    ...
    def _reject_idempotency_reuse(self, request: TraceRunRequest, digest: str) -> None:
        ...
        for run_directory in run_directories:
            ...
            if not run_directory.is_dir():
                continue
            events = self._read_events(run_directory.name)
            if not events:
                continue
            first = events[0]
            if first.idempotency_key == request.idempotency_key and (
                first.run_id != request.run_id or first.input_digest != digest
            ):
