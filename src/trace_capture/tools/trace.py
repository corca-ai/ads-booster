...
class _TraceRunArgs(BaseModel):
    ...
    component_artifact: str | None = Field(default=None, max_length=1_024)
...
class TraceRunTool:
    ...
    def descriptor(self) -> ToolDescriptor:
        return ToolDescriptor(
            ...
            parameters={
                ...
                "properties": {
                    "job": {"type": "string"},
                    "component_artifact": {"type": "string"},
    ...
    def execute(self, arguments: JsonObject, context: ToolContext) -> ToolResult:
        ...
        artifact = (
            resolve_workspace_path(context.workspace, parsed.component_artifact)
            if parsed.component_artifact is not None
            else None
        )
        if (
            job is None
            or state_root is None
            or capture_root is None
            or (parsed.component_artifact is not None and artifact is None)
        ):
            return ToolResult(
                ok=False,
        ...
        if artifact is not None:
            argv.extend(("--component-artifact", str(artifact)))
        try:
