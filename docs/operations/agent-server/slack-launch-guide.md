...
기존 설치에 Knowledge 설정이 없으면 새 버전의 관리형 시작 경로가 같은 tenant의 저장소와
설정을 자동으로 초기화한다. 이미 설정한 경로·기억·Slack 인증 정보는 보존하며, 일부 경로만
있는 불완전한 설정은 덮어쓰지 않고 오류로 처리한다. 매 업데이트마다 수동 초기화할 필요는
없다. 초기화된 설정은 이후 시작과 업데이트에서도 재사용한다.

Knowledge가 켜진 서비스의 health에는 `knowledge_worker: running`이 나온다. 해당 스레드가
종료되면 HTTP 503과 `status: degraded`, `knowledge_worker: stopped`를 반환한다. 개별 기억
저장 완료 여부는 작업과 선택 기록으로 별도 확인한다.
...
이 기능이 포함된 main의 자동 업데이트를 확인한 뒤 Slack 앱 관리에서
**Interactivity & Shortcuts → On**으로 설정하고 Request URL을 아래처럼 저장한다.

```text
https://marketing-agent.borca.ai/channels/slack/interactions
```

다른 도메인을 쓰면 호스트 이름만 바꾼다. 새 설치용 bootstrap/final manifest에는 이미 포함된다.
기존 앱은 서버 업데이트만으로 이 설정이 바뀌지 않으므로 한 번 직접 적용해야 한다.
권한 scope 추가나 Slack 앱 재설치는 필요 없다.

멘션/DM 요청 시 작업 상태와 `실행 중단` 버튼이 표시되고, 오래 걸리면 같은 메시지의 단계와
경과 시간이 5초마다 바뀌는지 확인한다. 완료 시 같은 메시지가 답변으로 바뀌고 버튼은 사라진다.
답변 생성 중 버튼을 눌러 `실행을 중단했습니다`가 표시되는지, 이후 새 요청이 동작하는지 확인한다.
요청자 또는 공유 채널의 승인 담당자만 중단할 수 있다. 이전 실행의 버튼은 새 실행을 중단하지 않는다.
이미 외부에 보낸 요청은 되돌리지 않는다. 완료가 확인된 이슈 링크는 남기며 결과가 불명확한
외부 요청은 확인 필요 상태로 유지한다. 대화 종료 명령 `종료`와 현재 실행 중단은 별개다.

## 기존 서버 로그인으로 이미지 생성하기

1. 이 기능이 포함된 main의 서버 자동 업데이트를 확인한다.
2. Slack 앱 관리 → OAuth & Permissions → Bot Token Scopes에 `files:write`를 추가한다.
3. Reinstall to Workspace를 눌러 추가 권한을 승인한다. 토큰이 바뀌면 서버 setup의 숨김 입력으로
   새 Bot Token을 설정하고 유휴 상태의 에이전트만 재시작한다.
4. 허용된 채널에서 봇을 멘션해 이미지의 주제·스타일·색상을 설명한다.
5. 봇이 제시한 이미지 생성 prompt를 검토한 뒤 정확한 승인 명령을 보낸다.
6. 작업 상태가 표시되고 같은 스레드에 PNG 초안이 첨부되는지 확인한다. 사용 전 직접 검토한다.

별도 이미지 API 키는 쓰지 않는다. 서버 사용자로 로그인한 Codex 계정과 설정 모델이
image_generation을 지원해야 한다. Mac 워커나 Appium은 이 일반 이미지 생성에 필요 없다.
이 새 생성 도구는 텍스트로 이미지 한 장을 만들며 DM에서는 제공하지 않는다.
기존의 선택적 이미지 검토·편집 도구 설정은 별도로 유지한다.
불확실한 생성·첨부는 자동 재시도하지 않는다. 첨부 실패 시 로컬 파일은 서비스의 images/
디렉터리에 유지되므로 권한과 Slack 첨부 여부를 먼저 확인한다.
