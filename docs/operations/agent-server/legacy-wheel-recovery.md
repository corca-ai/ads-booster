...
아래 버전·ZIP 이름·검증 상태·업데이트 정책은 해당 후보 시점의 기록이다. 현재 배포 상태나
운영 중인 설치본의 동작을 증명하지 않는다. 기존 수동 설치를 복구할 때 설치 SHA와 파일을
먼저 대조하며, 이후 후보 검증 경과는 [검증 기록](verification.md)에서 확인한다.
...
이 단계에서 전달한 패키지는 `trace-agent-server-setup-20260907-slack-agent.zip`이었다.
이전 autoupdate ZIP은 멘션·스레드·DM이 없는 후보다.

## 서버 Codex에게 전달

> 당시 전달한 자동 업데이트 패키지를 찾아 이 README와 verification.md를 읽고 설치해줘.
