...
| proof-first creative plan, execution ledger, independently re-derived conservative evaluation (including false worker winner/state/coverage rejection with no mutation), and exact structured learning applicability (including callback lineage re-derivation, legacy/mismatched exclusion, and nonmatching records before the bounded lookup) | `uv run pytest tests/marketing/test_hosted_creative_judgment.py tests/marketing/test_hosted_supervised_judgments.py tests/marketing/test_experiment_evaluation.py tests/marketing/test_marketing_agent_contracts.py tests/marketing/test_cloudflare_schema.py -q`; `node --test cloudflare/test/hosted-creative-plan.test.js cloudflare/test/hosted-marketing-agent.test.js cloudflare/test/marketing-agent-supervised-runtime.test.js` |
