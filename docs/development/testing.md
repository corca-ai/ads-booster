...
| `marketing/`, `cloudflare/` | Focused Python bridge, Queue decoder, D1 migration and hosted-workspace tests plus `cd cloudflare && npm run check`; parse the deployment workflow when it changes | Fresh-installed `trace-marketing simulate`; for hosted UI/context changes exercise root, public account create/switch/isolation, profile CRUD, immutable candidate profile snapshot, four-candidate morning/evening batch, structured feedback rule, candidate/review/R2 flow, submitted-candidate edit/delete, responsive UI, and one real Workers AI generation; after a qualifying `main` merge require the Actions migration/deploy and public context readbacks |
