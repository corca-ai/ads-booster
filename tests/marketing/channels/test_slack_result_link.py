...
from tests.marketing.agent_service.test_http_api import FailedReasoning
...
def test_normal_answer_and_readable_completed_status(
    tmp_path: Path,
) -> None:
    ...
    assert events.work_once(now=NOW)
    assert "상태: 완료" in str(messages[-1]["text"])
...
def test_interrupted_tool_plan_is_not_delivered_as_an_answer(tmp_path: Path) -> None:
    ...
    assert "실행 준비 중 처리가 중단됐습니다" in answer
    assert not adapter.inputs
    assert service.repository.records("team", run.run_id) == before
    assert "실행 준비 중 처리가 중단됐습니다" in events.summary(conversation, include_status=True)
...
def test_status_after_failed_followup_never_repeats_previous_answer(tmp_path: Path) -> None:
    events, messages = setup_events(tmp_path)
    receive(events)
    assert events.work_once(now=NOW)
    previous = str(messages[-1]["text"])
    service = events.commands.application.service
    service.reasoning = FailedReasoning()
    receive(events, type="message", text="이미지 만들어줘", ts="100.002", thread_ts="100.001")
    assert events.work_once(now=NOW)
    run = service.repository.list_runs("team")[0]
    before = service.repository.records("team", run.run_id)
    receive(events, type="message", text="상태", ts="100.003", thread_ts="100.001")
    assert events.work_once(now=NOW)
    for answer in (str(messages[-1]["text"]), events.commands.summary("team", run.run_id)):
        assert previous not in answer
        assert "요청을 해석하던 중 처리가 중단됐습니다" in answer
        assert "running" not in answer
        assert "운영자" not in answer
    assert service.repository.records("team", run.run_id) == before
