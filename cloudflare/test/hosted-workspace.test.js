...
test("Workers AI schema requires three complete candidates", () => {
  const schema = candidateResponseSchema();
  const candidates = schema.properties.candidates;

  assert.equal(candidates.minItems, 3);
...
test("workspace context assets expose data-driven country profiles", () => {
  ...
  assert.equal(WORKSPACE_CONTEXT_PROFILES.length, 4);
  assert.equal(WORKSPACE_CONTEXT_PROFILES.filter((profile) => profile.is_default).length, 1);
  assert.deepEqual(
    WORKSPACE_CONTEXT_PROFILES.map((profile) => normalizeContextProfile(profile).country),
    ["KR", "KR", "KR", "KR"],
  );
});
...
test("hosted context countries come from the packaged manifest", async () => {
  ...
  assert.equal(response.status, 200);
  assert.deepEqual(await response.json(), [{ country: "KR", language: "ko" }]);
});

test("profile creation fails before persistence when country documents are missing", async () => {
  const response = await handleHostedWorkspace(
    new Request("https://workspace.example/api/context-profiles", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: "JP starter",
        persona_id: "jp-starter",
        country: "JP",
        tone: "short",
        audience: "audience",
        situation: "situation",
  ...
  assert.match((await response.json()).detail, /JP 국가 context 문서/);
});
...
test("built public workspace has no login form and keeps candidate controls", async () => {
  ...
  const behavior = await readFile(new URL("../dist/static/workspace-live.js", import.meta.url), "utf8");
  ...
  assert.match(behavior, /method: editing \? "PATCH" : "POST"/);
  assert.match(
    behavior,
    /hostedCandidateControls && \(!editing \|\| editingCandidateContextChanged\)/,
  );
  assert.match(behavior, /method: "DELETE"/);
});
